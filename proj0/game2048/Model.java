...
public class Model extends Observable {
    ...
    public boolean tilt(Side side) {
        ...
        for (i = 0; i < this.board.size(); i++){
            int bottom = this.board.size();
            for (j = this.board.size()-2; j >= 0; j--){
                if (this.board.tile(i, j) != null) {
                    int value = this.board.tile(i, j).value();
                    for (int k = j + 1; k < bottom; k++){
                        if (this.board.tile(i, k) != null){
                            if(this.board.tile(i, k).value() == value){
                                this.board.move(i, k, this.board.tile(i, j));
                                changed = true;
                                this.score += 2 * value;
                                bottom = k;
                                break;
                                ...
                                this.board.move(i,k - 1, this.board.tile(i, j));
                                changed = true;
                                break;
                            }
                        }
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        ...
        if (emptySpaceExists(b)){
            ...
            while (i < b.size()-1){
                while (j < b.size()-1){
                    if (b.tile(i+1, j).value() == b.tile(i, j).value() || b.tile(i, j+1).value() == b.tile(i, j).value()){
                        return true;
                    }
                    j++;
                }
